...
const navigationItems: NavItem[] = [
  ...
  },
  {
    ...
    icon: Home,
    isActive: (pathname) => pathname === ROUTES.AUTHENTICATED.HOME || pathname === ROUTES.AUTHENTICATED.MURAL,
  ...
  },
  {
    ...
    href: ROUTES.AUTHENTICATED.MURAL,
    icon: Home,
    isActive: (pathname) => pathname === ROUTES.AUTHENTICATED.MURAL || pathname === ROUTES.AUTHENTICATED.HOME,
    // Este item será filtrado inversamente
  },
  {
    name: 'Agenda',
      ...
      <div className="container mx-auto px-4">
        <div className="flex h-20 items-start justify-around">
          {filteredNavigation.map((item) => {
            ...
            const isActive = item.isActive ? item.isActive(pathname) && !mainButton : pathname === item.href;
              ...
              <button
                ...
                className={clsx(
                  'group relative flex flex-col items-center justify-center space-y-1 rounded-lg px-3 py-2 transition-all duration-200',
                  {
                    ...
                    'bg-slate-950 -mt-4 rounded-full border border-slate-800 p-3 shadow-xl': mainButton,
                    'border-blue-500/40': mainButton && item.isActive && item.isActive(pathname),
                  },
                )}
              >
                <div
                  className={clsx('relative rounded-lg p-1 transition-all duration-200', {
                    'bg-blue-500/10 scale-110': isActive && !mainButton,
                    'group-hover:bg-slate-100 dark:group-hover:bg-slate-800': !isActive && !mainButton,
                    'bg-blue-500/10 scale-110 rounded-full p-2': mainButton,
                    'bg-blue-500/30': mainButton && item.isActive && item.isActive(pathname),
                  })}
                >
                  <Icon
                    className={clsx('h-5 w-5 transition-all duration-200', {
                      'text-blue-500': isActive || mainButton,
                ...
                {!mainButton && (
                  <span
                    className={`text-[10px] font-medium transition-all duration-200 ${
                      isActive ? 'text-blue-500 font-semibold' : 'text-slate-600 dark:text-slate-400'
                    }`}
                  >
                    {item.name}
                  </span>
                ...
                {isActive && !mainButton && (
                  <div className="absolute -top-0.5 left-1/2 h-1 w-1 -translate-x-1/2 transform rounded-full bg-blue-500" />
                )}
              </button>
            );
          })}
