...
export default function PrayerPage() {
  ...
  const filteredPrayers = prayers.filter(p => {
    ...
    const matchesSearch = p.request_text.toLowerCase().includes(searchTerm.toLowerCase()) || 
      ...
      const updatedPrayers = prayers.map(p => 
        p.id === prayer.id 
          ? { 
              ...
              viewed_by_pastor: { uid: currentUser?.uid || '', name: currentUser?.profile.full_name || 'Pastor' } 
            } 
          : p
      );
  ...
  const handleToggleArchive = (id: string) => {
    setPrayers(prayers.map(p => 
      p.id === id ? { ...p, is_archived: !p.is_archived } : p
    ));
  };
  ...
  const getStatusBadge = (status: PrayerRequest['status'], viewedBy?: any) => {
    switch (status) {
      case 'pending':
        return (
          <Badge variant="outline" className="flex items-center gap-1.5 border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-900/30 dark:bg-amber-900/20 dark:text-amber-400">
      ...
      case 'viewed':
        return (
          <Badge variant="outline" className="flex items-center gap-1.5 border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-900/30 dark:bg-blue-900/20 dark:text-blue-400">
      ...
      case 'praying':
        return (
          <Badge variant="outline" className="flex items-center gap-1.5 border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900/30 dark:bg-emerald-900/20 dark:text-emerald-400">
      ...
      case 'answered':
        return (
          <Badge variant="outline" className="flex items-center gap-1.5 border-purple-200 bg-purple-50 text-purple-700 dark:border-purple-900/30 dark:bg-purple-900/20 dark:text-purple-400">
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Pedidos de Oração</h1>
          <p className="text-slate-500 dark:text-slate-400">Compartilhe suas necessidades e interceda pelos irmãos.</p>
        </div>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-900/50">
                ...
                <Label htmlFor="confidential" className="cursor-pointer">
                  ...
                  <p className="text-xs text-slate-500">Visível apenas para os pastores da igreja.</p>
                </Label>
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setIsNewRequestOpen(false)}>Cancelar</Button>
              <Button onClick={handleCreateRequest} className="bg-rose-600 hover:bg-rose-700">Enviar Pedido</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </header>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        <Tabs value={viewTab} onValueChange={(v) => setViewTab(v as any)} className="w-full sm:w-auto">
        ...
        <div className="flex items-center gap-2">
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={`gap-2 ${dateFilter ? 'border-blue-500 text-blue-600' : ''}`}>
      ...
      <div className="grid gap-4">
        {filteredPrayers.map((prayer) => (
          ...
          >
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-slate-900 dark:text-white">
                      {prayer.is_confidential && !(isPastor || isSecretary || prayer.author_uid === currentUser?.uid) 
                        ? 'Pedido Confidencial' 
                        : prayer.author_name}
                    </h3>
                    {prayer.is_confidential && (
                      <Badge variant="secondary" className="text-[9px] uppercase tracking-tighter">Confidencial</Badge>
                    )}
                  </div>
                  <p className="text-xs text-slate-500">
                    {new Date(prayer.created_at).toLocaleDateString('pt-BR')} às {new Date(prayer.created_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                  </p>
                </div>
              </div>
                ...
                {(prayer.author_uid === currentUser?.uid || isPastor) && (
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleToggleArchive(prayer.id); }}>
                        {prayer.is_archived ? (
                          <>
            ...
            {prayer.pastor_response && (
              <div className="mt-4 flex items-start gap-3 rounded-xl bg-blue-50/50 p-3 dark:bg-blue-900/10">
                <MessageCircle className="mt-0.5 h-4 w-4 text-blue-500" />
                <div className="flex-1">
                  <p className="text-xs font-bold text-blue-700 dark:text-blue-400">Resposta do Pastor:</p>
                  <p className="text-xs text-slate-600 dark:text-slate-400">{prayer.pastor_response}</p>
                </div>
              </div>
            )}
          </div>
        ...
        {filteredPrayers.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="mt-4 text-lg font-bold text-slate-900 dark:text-white">Nenhum pedido encontrado</h3>
            ...
            <>
              <DialogHeader>
                <div className="flex items-center gap-3">
                  ...
                  <DialogTitle className="text-xl font-bold">Pedido de {selectedPrayer.author_name}</DialogTitle>
                </div>
              </DialogHeader>

              <div className="space-y-6 py-4">
                <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-900/50">
                  <p className="text-sm font-medium text-slate-500 mb-2 uppercase tracking-wider">O Pedido:</p>
                ...
                {isPastor ? (
                  <div className="space-y-3">
                    ...
                    <textarea 
                      ...
                      className="min-h-[100px] w-full rounded-xl border border-blue-100 bg-blue-50/30 p-3 text-sm focus:border-blue-500 focus:outline-none dark:border-blue-900/30 dark:bg-blue-900/10"
                  ...
                  <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4 dark:border-blue-900/30 dark:bg-blue-900/20">
                    <p className="text-xs font-bold text-blue-700 dark:text-blue-400 mb-1 flex items-center gap-2">
              ...
              <DialogFooter className="gap-2">
                <Button variant="outline" onClick={() => setIsViewRequestOpen(false)}>Fechar</Button>
                {isPastor && (
                  <Button onClick={handlePastorResponse} className="bg-blue-600 hover:bg-blue-700">
                    Enviar Resposta
                  </Button>
                )}
              </DialogFooter>
