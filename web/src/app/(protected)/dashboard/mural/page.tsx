...
export default function MuralPage() {
  ...
  const filteredPosts = posts.filter(post => {
    ...
    const isFromMyGroup = currentUser && post.target_groups.some(group => (currentUser.sub_groups || []).includes(group));
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Feed da Igreja</h1>
          <p className="text-slate-500 dark:text-slate-400">Fique por dentro de tudo que acontece na Hebrom.</p>
        </div>
            ...
            >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {dayjs(post.created_at?.seconds ? post.created_at.seconds * 1000 : post.created_at).fromNow()}
                    </p>
                  </div>
                </div>
              ...
              {post.media_url && (
                <div className="mt-4 overflow-hidden rounded-xl border border-slate-100 dark:border-slate-800">
                  <img src={post.media_url} alt={post.title} className="w-full object-cover max-h-72" />
                </div>
              )}

              <div className="mt-4 space-y-2">
                <h4 className="text-lg font-bold text-slate-800 dark:text-slate-100">{post.title}</h4>
