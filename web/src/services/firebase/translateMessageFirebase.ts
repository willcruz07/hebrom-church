...
const translateAuthenticationErrorMessage: IAuthTranslate = {
    ...
    'auth/invalid-action-code':
        'O código da ação é inválido. Isso pode acontecer se o código estiver mau formado ou já tiver sido usado.',
    'auth/user-disabled':
...
export const getFirebaseErrorMessageTranslation = (
    messageToTranslate: any,
    messageDefault: string
): string => {
    const errorCode = messageToTranslate?.code || '';

    if (errorCode) {
        return translateAuthenticationErrorMessage[errorCode] || messageDefault;
    }
